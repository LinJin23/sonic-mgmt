...
def setup_vnet_cfg(duthost, localhost, cfg_facts):
    ...
    cfg_t0.setdefault("DEVICE_METADATA", {})
    cfg_t0["DEVICE_METADATA"].setdefault("localhost", {})
    cfg_t0["DEVICE_METADATA"]["localhost"]["deployment_id"] = "26"
    ...
    duthost.shell(
        ...
        "-c 'ip prefix-list IPV4_DOWNSTREAM_PREFIXES_VNET_Vnet2 seq 15 permit 0.0.0.0/0 le 32' "
        "-c 'router bgp {} vrf Vnet2' "
        "-c 'address-family ipv4 unicast' "
        "-c 'neighbor BGPSLBPassive allowas-in 1' "
        "-c 'no neighbor BGPSLBPassive maximum-prefix 100' "
        "-c 'no neighbor BGPSLBPassive default-originate' "
        .format(bgp_asn))
