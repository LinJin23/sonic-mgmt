"""Self-contained BBR helpers for tests/closa/.

Closa-local copy of the symbols originally provided by
``tests/bgp/bgp_bbr_helpers.py``.  The repo CI
(``.azure-pipelines/dependency_check/dependency_check.py``) forbids
cross-feature imports (``tests/closa/*`` may not import from
``tests/bgp/*``), and the closa policy forbids modifying public files
under ``tests/bgp/``; this in-place copy is the intersection.

If the upstream public ``bgp_bbr_helpers`` ever fixes a bug, this file
